...
function obs_stat(tag) {
    return rxjs_1.Observable.bindCallback(fs.stat, function (err, stat) { return [err, stat, tag]; });
}
function obs_readdir(tag) {
    return rxjs_1.Observable.bindCallback(fs.readdir, function (err, files) { return [err, files, tag]; });
}
function obs_readFile(tag) {
    return rxjs_1.Observable.bindCallback(fs.readFile, function (err, data) { return [err, data, tag]; });
}
function bootNode($tw) {
                ...
                if (stat.isDirectory()) {
                    return obs_readdir()(filepath).switchMap(function (_a) {
                        ...
                        if (files.indexOf("tiddlywiki.files") !== -1) {
                            ...
                            return rxjs_1.Observable.from(files.filter(function (file) {
    ...
    */
    function loadTiddlersFromSpecification(filepath, excludeRegExp) {
        ...
        return obs_readFile()(filepath + path.sep + "tiddlywiki.files", "utf8").switchMap(function (res) {
            ...
            return rxjs_1.Observable.from(filesInfo.tiddlers).concatMap(function (tidInfo) {
            ...
            }).concat(rxjs_1.Observable.from(filesInfo.directories).concatMap(function (dirInfo) {
                if (typeof dirInfo === "string") {
                    ...
                    return obs_readdir()(dirPath).switchMap(function (_a) {
                        ...
                        return rxjs_1.Observable.from(files.filter(function (filename) {
    ...
    */
    function loadPluginFolder(filepath, excludeRegExp) {
        ...
        return obs_stat()(filepath).switchMap(function (_a) {
            var err = _a[0], stat = _a[1];
            if (err || !stat.isDirectory())
                return rxjs_1.Observable.throw(err || { message: "Directory required" });
    ...
    */
    function findLibraryItem(name, paths) {
        return rxjs_1.Observable.from(paths).concatMap(function (itemPath) {
            var pluginPath = path.resolve(path, "./" + name);
    ...
    */
    function loadPlugins(plugins, libraryPath, envVar) {
        if (plugins) {
            ...
            return rxjs_1.Observable.from(plugins).concatMap(function (plugin) {
    ...
    function loadWikiTiddlers(wikiPath, options) {
        ...
        return obs_readFile()(wikiInfoPath, "utf8").concatMap(function (_a) {
            ...
            if (wikiInfo.includeWikis) {
                ...
                return rxjs_1.Observable.from(wikiInfo.includeWikis).concatMap(function (info) {
            ...
            return rxjs_1.Observable.concat([
            ...
            return rxjs_1.Observable.from([
                ...
                if (err)
                    ...
                    return rxjs_1.Observable.from(pluginFolders.map(function (a) { return [a, pluginPath]; }));
            }).concatMap(function (_a) {
    ...
    function loadTiddlersNode() {
        ...
        return rxjs_1.Observable.concat(loadTiddlersFromPath($tw.boot.bootPath).do(function (tiddlerFile) {
            $tw.wiki.addTiddlers(tiddlerFile.tiddlers);
